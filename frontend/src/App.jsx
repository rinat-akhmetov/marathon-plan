  ...
  return (
    <div style={{minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-start', background:'none'}}>
      <header style={{width:'100%', padding:'2rem 0 1rem 0', textAlign:'center', position:'relative'}}>
        <div style={{position:'absolute', top:24, right:32, display:'flex', alignItems:'center', gap:'0.75rem'}}>
          <button type="button" className="button" onClick={() => window.location.href = '/login'} style={{fontSize:'0.9rem'}}>Sign in</button>
          <button aria-label="Toggle dark mode" onClick={handleToggle} style={{background:'none', border:'none', cursor:'pointer', padding:0}}>
            {dark ? (
